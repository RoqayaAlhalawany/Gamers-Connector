...
export class ReviewPeerComponent implements OnInit {
  listOfReviewQuestions: ReviewQuestionsPayload[]; // Array to store the review questions
  listOfReviewAnswers: SendReviewAnswersPayload[]; // Array to store the review answers
  otherPlayerDetails: GetOthersDetailsPayload; // Details of the other player
  username: string; // Username of the other player
  index: number; // Index counter

  constructor(
    private reviewQuestions: GetReviewPeerQuestionsService,
  ...
  ngOnInit(): void {
    // Fetch review questions when the component initializes
  ...
  // Create a form group for the review answers
  ...
  // Function to submit the review answers
  submitReviewAnswers() {
    console.log(this.listOfReviewQuestions);
    console.log(this.listOfReviewAnswers);

    // Retrieve and store the review answers
